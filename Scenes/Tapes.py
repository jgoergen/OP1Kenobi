...
class Tapes():
    ...
    def __init__(self, core, audio, video, input):
        ...
        self.g = 0
        self.currentIndex = 0
        self.lastDirectories = [Config.OP1USBMountDir + "/tape"]
        ...
        if self.op1Present:
            ...
            if self.menu == 1:
                ...
                if k1:
                    ...
                    if self.local:
                        targetDirectory = Config.OP1USBMountDir + "/tape/"
                        path = path.replace(Config.MediaDirectory + "/tapes/", "")
                    else:
                        targetDirectory = Config.MediaDirectory + "/tapes/"
                        path = path.replace(Config.OP1USBMountDir + "/tape/", "")

                    self.core.ForceDirectory(targetDirectory + path)

                    # copy
                    print("Tapes:: Copying file " + currentObject)
                    print("Tapes:: To " + targetDirectory + path + "/" + self.currentFileName)
                    self.core.CopyFile(
                        currentObject,
                ...
                if k2:
                    if self.local:
                        ...
                        self.local = True
                        self.lastDirectories = [Config.MediaDirectory + "/tapes"]
