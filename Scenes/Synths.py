...
class Synths():
    ...
    def __init__(self, core, audio, video, input):
        ...
        self.g = 0
        self.currentIndex = 0
        self.lastDirectories = [Config.OP1USBMountDir + "/synth"]
        ...
        if self.op1Present:
            ...
            if self.menu == 1:
                ...
                if k1:
                    ...
                    if self.local:
                        targetDirectory = Config.OP1USBMountDir + "/synth/"
                        path = path.replace(Config.MediaDirectory + "/synths/", "")
                    else:
                        targetDirectory = Config.MediaDirectory + "/synths/"
                        path = path.replace(Config.OP1USBMountDir + "/synth/", "")

                    self.core.ForceDirectory(targetDirectory + path)

                    # copy
                    print("Synths:: Copying file " + currentObject)
                    print("Synths:: To " + targetDirectory + path + "/" + self.currentFileName)
                    self.core.CopyFile(
                        currentObject,
                ...
                if k2:
                    if self.local:
                        ...
                        self.local = True
                        self.lastDirectories = [Config.MediaDirectory + "/synths"]
